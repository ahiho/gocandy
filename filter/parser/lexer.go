...
func (l *Lexer) Scan() (Position, Token, string) {
	pos, tok, val := l.scan()
	if tok.String() == ILLEGAL.String() {
		return pos, ILLEGAL, ""
	}
	if IsTokenComparator(tok.String()) {
		return pos, tok, tok.String()
	}

	// if IsValComparator(val) {
	// 	fmt.Println("DEBUG 22222222", tok, val)
	// 	return pos, tok, val
	// }
...
func IsTokenComparator(lit string) bool {
	tokens := []string{"=", "!", ">", "<", "<=", ">=", "!=", "(", ")", ",", "LIKE"}
	return lo.Contains(tokens, lit)
}

func IsValComparator(val string) bool {
	vals := []string{"IN", "NOT"}
